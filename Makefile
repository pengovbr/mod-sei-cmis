.PHONY: .env help clean dist all install destroy up up-alfresco prepare-alfresco update down test test-functional test-functional-parallel test-unit bash_org1 bash_org2 verify-config check-alfresco check-cmis-api
...
ALFRESCO_COMPOSE_DIR = ../../acs-deployment/docker-compose
...
check-alfresco: ## Target de apoio. Verifica se o Alfresco está respondendo
	@echo ""
	@echo "Testando se o Alfresco está respondendo..."
	ALFRESCO_PORT=$$(grep -E '^ALFRESCO_PORT=' $(PEN_TEST_FUNC)/.env 2>/dev/null | cut -d'=' -f2 || echo "8080"); \
	if [ -z "$$ALFRESCO_PORT" ]; then ALFRESCO_PORT="8080"; fi; \
	echo "ALFRESCO_PORT: $$ALFRESCO_PORT"; \
	for i in `seq 1 30`; do \
	    echo "Tentativa $$i/30";  \
		if curl -s -f http://localhost:$$ALFRESCO_PORT/alfresco/api/-default-/public/alfresco/versions/1/probes/-ready- > /dev/null 2>&1 || \
		   curl -s -f http://localhost:$$ALFRESCO_PORT/alfresco/ > /dev/null 2>&1; then \
				echo 'Alfresco está respondendo!' ; \
				break ; \
		fi; \
		sleep 5; \
	done; \
	if ! curl -s -f http://localhost:$$ALFRESCO_PORT/alfresco/api/-default-/public/alfresco/versions/1/probes/-ready- > /dev/null 2>&1 && \
	   ! curl -s -f http://localhost:$$ALFRESCO_PORT/alfresco/ > /dev/null 2>&1; then \
		echo 'Alfresco não está respondendo. Verifique...'; exit 1 ; \
	fi;

check-cmis-api: ## Target de apoio. Verifica se a API CMIS está respondendo
	@echo ""
	@echo "Testando se a API CMIS está respondendo..."

	CMIS_PORT=$$(grep -E '^CMIS_API_HOST_PORT=' $(PEN_TEST_FUNC)/.env 2>/dev/null | cut -d'=' -f2); \
	if [ -z "$$CMIS_PORT" ]; then CMIS_PORT="8085"; fi; \
	echo "CMIS_PORT: $$CMIS_PORT"; \
	CMIS_URL="http://localhost:$$CMIS_PORT"; \
	echo "CMIS_URL: $$CMIS_URL"; \
	echo "curl -s -f $$CMIS_URL/api/health > /dev/null 2>&1" && \
	for i in `seq 1 15`; do \
	    echo "Tentativa $$i/15";  \
		if curl -s -f http://localhost:$$CMIS_PORT/api/health > /dev/null 2>&1 || \
		   curl -s -f http://localhost:$$CMIS_PORT/api/ > /dev/null 2>&1 || \
		   curl -s -f http://localhost:$$CMIS_PORT/ > /dev/null 2>&1; then \
				echo 'API CMIS está respondendo!' ; \
				break ; \
		fi; \
		sleep 3; \
	done; \
	if ! curl -s -f http://localhost:$$CMIS_PORT/api/health > /dev/null 2>&1 && \
	   ! curl -s -f http://localhost:$$CMIS_PORT/api/ > /dev/null 2>&1 && \
	   ! curl -s -f http://localhost:$$CMIS_PORT/ > /dev/null 2>&1; then \
		echo 'API CMIS não está respondendo. Verifique...'; exit 1 ; \
	fi;
...
install: check-isalive check-alfresco check-cmis-api
	$(CMD_COMPOSE_FUNC) up -d	
	@CMIS_PORT=$$(grep -E '^CMIS_API_HOST_PORT=' $(PEN_TEST_FUNC)/.env 2>/dev/null | cut -d'=' -f2 || echo "8085"); \
	ALFRESCO_PORT=$$(grep -E '^ALFRESCO_PORT=' $(PEN_TEST_FUNC)/.env 2>/dev/null | cut -d'=' -f2 || echo "8080"); \
	echo "✅ Alfresco disponível em http://localhost:$$ALFRESCO_PORT/alfresco"; \
	echo "✅ CMIS API disponível em http://localhost:$$CMIS_PORT"
...
prepare-alfresco: ## Prepara o repositório do Alfresco (clona se necessário)
	@echo "📦 Verificando se o repositório do Alfresco está disponível..."
	@if [ ! -d "$(ALFRESCO_COMPOSE_DIR)" ]; then \
		echo " Repositório não encontrado. Clonando acs-deployment..."; \
		echo "   Executando: git clone https://github.com/Alfresco/acs-deployment.git"; \
		cd ../../ && git clone https://github.com/Alfresco/acs-deployment.git || { \
			echo "Erro ao clonar o repositório do Alfresco. Verifique sua conexão com a internet."; \
			exit 1; \
		}; \
		echo "Repositório clonado com sucesso!"; \
	else \
		echo "Repositório do Alfresco já existe em $(ALFRESCO_COMPOSE_DIR)"; \
	fi

up-alfresco: prepare-alfresco ## Sobe o container do Alfresco (agora integrado no docker-compose funcional)
	@echo "Alfresco será levantado junto com os outros containers do docker-compose funcional"

up: .env prepare-upload-tmp 
# Em alguns BDs os containers demoram alguns segundos para ficarem estáveis.
	@echo "Subindo todos os containers (Alfresco, CMIS API, SEI org1 e org2)..."
	@make up-alfresco || echo "Aviso sobre Alfresco, mas continuando..."
	$(CMD_COMPOSE_FUNC) up -d --build
	@echo "⏳ Aguardando Alfresco estar estável (pode levar alguns minutos)..."
	@sleep 5
	...
	@echo "Verificando se CMIS API está respondendo..."
	@make check-cmis-api || echo " CMIS API ainda não está respondendo, mas continuando..."
...
prepare-upload-tmp:
	@if [ ! -d "$(PEN_TEST_FUNC)/.tmp" ]; then \
		...
		echo "\n   Diretório .tmp já existe, pulando download.\n"; \
	fi
